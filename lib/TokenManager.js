...
function setRefreshTimeout(sdk, tokenMgmtRef, storage, key, token) {
  ...
  var refreshTimeout = setTimeout(function() {
    ...
    if (tokenMgmtRef.autoRefresh) {
      getAsync(sdk, tokenMgmtRef, storage, key);
...
function refresh(sdk, tokenMgmtRef, storage, key) {
  ...
  if (!tokenMangamentRef.refreshPromise) {
    tokenMangamentRef.refreshPromise = sdk.token.refresh(token)
    .then(function(freshToken) {
      add(sdk, tokenMgmtRef, storage, key, freshToken);
      tokenMgmtRef.emitter.emit('refreshed', key, freshToken, token);
      tokenMangamentRef.refreshPromises.forEach(deferred => deferred.resolve(freshToken))
      tokenMgmtRef.refreshPromises[key] = [];
      tokenMangamentRef.refreshPromise = null;
    })
    .fail(function(err) {
      ...
      throw err;
      tokenMangamentRef.refreshPromises.forEach(deferred => deferred.rejec(err))
      tokenMgmtRef.refreshPromises[key] = [];
      tokenMangamentRef.refreshPromise = null;
    });
  }
