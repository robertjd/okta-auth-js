...
function verifyToken(sdk, token, clientId, nonce, ignoreSignature) {
  var aud = clientId || sdk.options.clientId;
  ...
  .then(function() {
    ...
    oauthUtil.validateClaims(sdk, jwt.payload, aud, token.issuer, nonce);

    // If the browser doesn't support native crypto or we choose not
    // to verify the signature, bail early
    if (ignoreSignature || sdk.options.ignoreSignature || !sdk.features.isTokenVerifySupported()) {
      return token;
    }
...
function handleOAuthResponse(sdk, oauthParams, res, urls) {
  ...
  var ignoreSignature = oauthParams.ignoreSignature || sdk.options.ignoreSignature;
  ...
  .then(function() {
    ...
    if (res['id_token']) {
      ...
      return verifyToken(sdk, idToken, clientId, oauthParams.nonce, ignoreSignature)
      .then(function() {
